...
tools: get_trace, find_traces, list_suites, get_suite, create_suite, add_to_suite, update_expected_output, list_evaluators, create_evaluator, set_suite_evaluators, start_test_run, get_case_results, compare_runs, list_theories, submit_optimization_theory, await_actions
---
...
## 4. Write the failing case

Add the trace as a case whose `expectedOutput` is what the agent SHOULD have said — a correction:
...
## 5. Give the suite a judge that can score the rule

`list_evaluators` first — the digest carries each agentic judge's instructions, so you can tell
whether one already covers this behavior. Reuse a fit. Otherwise `create_evaluator` with kind
...
## 6. Red — run it, and require the failure
...
## 7. Green — one change, A/B-validated, proven on your case
